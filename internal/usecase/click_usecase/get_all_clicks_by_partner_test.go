...
func TestGetAllClicksByPartner(t *testing.T) {
	var offerID int64 = 1
	...
	tests := []struct {
	...
	}{
		{
			...
			before: func(f useCaseMocks) {
				...
				f.repoClick.EXPECT().GetAllClicksByPartner(partnerID).Return([]*domain.Click{click})
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			e := uc.GetClicksByPartner(tt.args.req)
